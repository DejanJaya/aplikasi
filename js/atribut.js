...
$(".open").removeClass('open');
 $("#menu_pertama"+menuaktif).addClass('open');
	...
	$('#form_periode')
	 ...
	 function simpanDataperiode(){
		// alert(page)
 	 var data = $('#form_periode').serializeArray();
			  $.ajax({
				  type: "POST",
				  url: ""+page+"/simpanData",
				  data: data,
   ...
   function statusFormat(value, row, index) {
    ...
    window.operateEvents = { 
		...
		'click #kontrak': function (e, value, row, index) {
            kontakUbah(row);
        },'click #editmnl': function (e, value, row, index) {
				editFormmanual(row);
        },'click #appdet': function (e, value, row, index) {
				approveData(row.id);
        },'click #removeapp': function (e, value, row, index) {
			var setda=row.status;
			if(setda=="1"){
				bootbox.alert("Data Sudah di Approve");
			}else{
				hapusData(row.id);
			}
